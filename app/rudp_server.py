...
def setup():
    ...
    while True:
        ...
        try:
            ...
            received_packet = pack_data(NAK, seq_num, 0, 0, 0, 0, "")
...
def handle(client_address, control_bits, seq_num, server_socket):
    ...
    if control_bits == SYN:
        ...
        seq_num += 1
        last_bit = SYN_ACK
        sent_packet = pack_data(SYN_ACK, seq_num, 0, 0, 0, 0, "SYN-ACK")
        ...
        if control_bits == PSH:
            ...
            for chunk in chunks:
                if size - i == 0:
                    ...
                    last_chunk = 0
                time.sleep(1)
                sent_packet = pack_data(control_bits=PSH_ACK, seq_num=seq_num,
                                        chunk_num=size - i, retransmission=0,
                                        total_chunks=size, last_chunk=last_chunk,
                                        data=chunk)
                server_socket.sendto(sent_packet, client_address)
                ...
                while waiting_for_ack:
                    ...
                    reset_timeout(server_socket)
                    try:
                        control_bits, data_size, seq_num, total, chunk_num, retransmission_flag, last_chunk_flag, data = unpack_data(received_packet)
                        if control_bits == ACK:
            ...
            while client_flag:
                sent_packet = pack_data(control_bits=FIN_ACK, seq_num=seq_num,
                                        chunk_num=0, retransmission=0,
                                        total_chunks=0, last_chunk=0,
                                        data="FIN_ACK")
                server_socket.sendto(sent_packet, client_address)
                try:
                    received_packet, client_address = server_socket.recvfrom(buffer_size)
                    ...
                    sent_packet = pack_data(FIN_ACK, seq_num, 0, 0, 0, 0, "FIN_ACK")
                    server_socket.sendto(sent_packet, client_address)
                    time_out += 1
                    server_socket.settimeout(time_out)
                    received_counter = 0
...
def pack_data(control_bits, seq_num, chunk_num, retransmission, total_chunks, last_chunk, data):
    ...
    print(f"Chunk:{chunk_num} Data_size:{len(data)} Retransmission:{retransmission} Last_chunk:{last_chunk}")
    print(f"Data:{data}")
    return packet
...
def unpack_data(packet):
    ...
    print(f"data:{data}")
    print("---------------------------------------")
    return control_bits, data_size, seq_num, total, chunk_num, retransmission_flag, last_chunk_flag, data
