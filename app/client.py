        ...
        if 13 < int(user_input) or int(user_input) < 1:
            ...
            match int(user_input):
                ...
                case 7:
                    ...
                    game_id = validate_id_check()
                    request = delete_game_message(game_id=game_id)
                    handle_respond(client_socket, request)
                case 8:
...
def validate_score_check() -> float:
    ...
    while True:
        try:
            score = float(input("Please valid score 0<=score<=10"))
        except ValueError:
            print("Invalid input.")
            continue
...
def validate_category_check() -> str:
    category = input("JRPG, Adventure, Shooter, Action, Fighting, Platformer, RPG, Survival, Sport,MMO")
    while True:
